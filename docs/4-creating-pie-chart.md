...
2. Let’s load this word frequency data into Tableau. Go to Data->New Data Source.  
    ...
    Pick RomeoAndJulietWordFrequenciesByAct.xls.  
...
3. You can see that it is a simple spreadsheet that counts how many times a term/word came up in each act of the play. Let’s click on new worksheet to get started. Let’s rename this one to “Pie”.
...
6. Hold down the Control Key and click on Act and Count (hold down the Command Key if on Mac), and then expand the Show Me tab. I see that pie chart is one of the recommendations. Let’s pick it.
...
8. Next, let’s label it. Drag Act on to Label, and then Count on to Label. Then click on Label, and click on the ... icon next to the Text field to edit the label.
    ...
    Add the word “Act” to just before the <**Act**> number to clarify the label, and then click on OK. Then click outside the pop-up to close the window.  
...
10. We could also format the percentages to round to whole numbers. Right click on the SUM (Count) Label Pill again, and this time select Format.  
    ...
    From the Format window that appears on the left, in the Pane tab, under the Default section, from the drop-down for Numbers, select Percentage and then change the number of Decimal places to zero. Then close the window.  
